...
const SeatChart = ({ occasion, ticketMaster, provider, setToggle }) => {
    const [seatsTaken, setSeatsTaken] = useState(false)
    const [hasSold, setHasSold] = useState(false)

    const getSeatsTaken = async () => {
        const seatsTaken = await ticketMaster.getSeatsTaken(occasion.id);
    ...
    const buyHandler = async (_seat) => {
        ...
        const transaction = await ticketMaster.connect(signer).mint(occasion.id, _seat, { value: occasion.cost })
